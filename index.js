...
const patBot = [
  "ha acariciado a {bot} 🥺🐺","le ha dado pat pat a {bot} 💚","está mimando a {bot} hasta dormirlo 😴",
  "le rascó las orejitas a {bot} 🐾","le dio muchos mimos a {bot} 💤","está consintiendo a {bot} con pat pat ✨",
  "le dio su dosis diaria de pat a {bot} 💚","abrazó a {bot} con pat pat 🫂"
]
const patOtros = ["le dio pat pat a {target} 🥰","acarició a {target} 💚","está mimando a {target} 😳","le rascó las orejitas a {target} 🐾","le dio muchos pats a {target} ✨","consintió a {target} con mucho amor 💖"]

async function askAI(prompt){
  try{
    const res = await fetch(`https://text.pollinations.ai/${encodeURIComponent(prompt)}?model=openai`, { headers: { "User-Agent":"Mozilla/5.0" }, signal: AbortSignal.timeout(20000) })
    return (await res.text()).slice(0, 1500)
  }catch{ return "⚠️ La IA está ocupada, intenta de nuevo en 5 seg." }
}
  ...
  sock.ev.on('messages.upsert', async ({messages})=>{
    ...
    if(lower.startsWith('#pat')){
      ...
      if(isBot){
        ...
        savePats()
        let frase = patBot[Math.floor(Math.random()*patBot.length)].replace(/{bot}/g, BOT_NAME)
        await sock.sendMessage(from,{text:`*${BOT_NAME} v2.0 in operation!*\n🐺 *${sender}* ${frase}\n\n📊 Llevas *${pats[senderJid].count}* pats a ${BOT_NAME}`})
      }else{
        let frase = patOtros[Math.floor(Math.random()*patOtros.length)].replace(/{target}/g, `@${jid.split('@')[0]}`)
    ...
    if(lower==='#pats' || lower==='#pats top'){
      ...
      if(mine) txt += `\nTu: ${mine.count} pats`
      txt += `\n\n_Premio al top 1: bot personalizado por ☾Edlegoshi☽_`
      await sock.sendMessage(from,{text:txt})
    }

    if(lower.startsWith('#b ')){
      const prompt = textRaw.slice(3).trim()
      if(!prompt) return sock.sendMessage(from,{text:"Usa: #b que es 5+5 o #b traduceme hello"})
      if(/^[0-9+\-*/().% ]+$/.test(prompt)){
        try{
